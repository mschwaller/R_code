t_fun2 <- function(sfacs){
  ...
  # since t_fun2 is passed to the function optim_nm there isn't any way to pass the variables 
  # below to it, so we access them from the R global environment using get
  tt <- get("TT", envir = .GlobalEnv)
  tr <- get("TR", envir = .GlobalEnv)
  ts <- get("TS", envir = .GlobalEnv)
  colony_sc_matrix <- get("colony_phis", envir = .GlobalEnv)
  general_sc_matrix <- get("general_phis", envir = .GlobalEnv)
  ...
  general_is_in <- 0
  general_is_out <- 0
  min_r_dist <- 10000.0 # use this to find the general features pixel with the smallest d-value
  for (ic in 1:i_end) {
    sc_array[1:num_phis] <- general_sc_matrix[ic,] 
    result <- t_invert %*% sc_array # last element of result should be a 1, see Equations 28 & 29
    r_dist <- sqrt(t(result[1:num_phis]) %*% result[1:num_phis]) # Equation 30, SSQ of the 1st n-1 elements
    if (r_dist < min_r_dist) {min_r_dist <- r_dist} # find the general features pixel with the smallest d-value
  ...
  print(c("iteration ", toString(iteration_counter)))
  print(c("tau = ", toString(tau)))
  print(c("optimization criterion value = ", toString(tau_factor)))
  ...
  print(c("nearest general pixel distance: ", toString(min_r_dist)))
  print(" ")
