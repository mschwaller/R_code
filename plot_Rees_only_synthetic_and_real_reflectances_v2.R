...
skip_casey = 9 # skip the last 9 rows of the input reflectance values (these have Casey's observations)
...
num_reese_rows <- length(Reese_synthetic_rhos[,1]) - skip_casey
num_DART_rows <- length(all_DART_rhos[,1])
id_vec<- c("a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q")
landsat_rho_frame <- data.frame("B1"=TOA_colony_matrix[,1]/10000, "B2"=TOA_colony_matrix[,2]/10000, "B3"=TOA_colony_matrix[,3]/10000, "B4"=TOA_colony_matrix[,4]/10000, 
                                "B5"=TOA_colony_matrix[,5]/10000, "B6"=TOA_colony_matrix[,6]/10000, "source"=rep("Landsat", num_landsat_rows), "color"=rep("red", num_landsat_rows), 
                                "pch"=rep(1, num_landsat_rows), "point_id"=rep(" ", num_landsat_rows), stringsAsFactors=FALSE) # pch = point character = symbol; 1 = open circle
Reese_rho_frame <- data.frame("B1"=Reese_synthetic_rhos[2:num_reese_rows,1], "B2"=Reese_synthetic_rhos[2:num_reese_rows,2], "B3"=Reese_synthetic_rhos[2:num_reese_rows,3],
                              ...
                              "source"=rep("Reese", num_reese_rows-1), "color"=rep("blue", num_reese_rows-1), 
                              "pch"=rep(0, num_reese_rows-1), "point_id"=rep(" ", num_reese_rows-1), stringsAsFactors=FALSE)  # pch = 0 = open square
all_DART_rho_frame <- data.frame("B1"=all_DART_rhos[,1], "B2"=all_DART_rhos[,2], "B3"=all_DART_rhos[,3], "B4"=all_DART_rhos[,4], 
                        "B5"=all_DART_rhos[,5], "B6"=all_DART_rhos[,6], "source"="REESE_DART_TOA", "color"="black", 
                        "pch"=19, "point_id"=id_vec, stringsAsFactors=FALSE) # pch = 19 = solid circle
...
for (ip in 1:15) {
  ...
  plot(landsat_rho_frame[[xname]],landsat_rho_frame[[yname]], col=landsat_rho_frame$color, xlim=c(0.0,0.6), ylim=c(0.0,0.6),
       xlab=xname, ylab=yname, pch=landsat_rho_frame$pch)
  points(landsat_rho_frame[[xname]]+0.02,landsat_rho_frame[[yname]], type="p", pch=landsat_rho_frame$point_id)
